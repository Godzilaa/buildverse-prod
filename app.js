...
/* ================= 14. SUPABASE BACKEND SYSTEM ================= */

const supabaseConfig = {
    url: "https://irkgaolctykjyhnsxryg.supabase.co", // Replace with your real Supabase URL
    anonKey: "sb_publishable_9pmTjgxI_dyNrfeMERIZSA_Ktriie3A" // Replace with your real Supabase Anon Key
};
...
// Helper functions for key casing conversions between JavaScript camelCase and PostgreSQL snake_case
...
function mapKeysToSnake(obj) {
    ...
    for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
            let value = obj[key];
            if (value && typeof value === 'object' && value.constructor && (value.constructor.name === 'FieldValue' || value.constructor.name === 'Timestamp')) {
                continue;
            }
...
function mapKeysToCamel(obj, colName) {
    if (!obj || typeof obj !== 'object' || Array.isArray(obj)) return obj;
    const newObj = {};
    for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
            let newKey = toCamelCase(key);
            if (key === 'id' && (colName === 'users' || colName === 'parents')) {
                newKey = 'uid';
            }
...
// Mock compat definition to avoid breaking existing Firebase utility calls in core form logic
const firebase = {
    firestore: {
        FieldValue: {
            serverTimestamp: () => 'now()'
...
class SupabaseFirestoreAdapter {
    ...
    collection(colName) {
        return new SupabaseCollection(this.client, colName);
    }
    ...
    async runTransaction(cb) {
        ...
        try {
            ...
            console.error("Supabase transaction simulation error:", e);
            throw e;
        }
    }
}
...
class SupabaseCollection {
    ...
    doc(docId) {
        return new SupabaseDocument(this.client, this.colName, docId);
    }
    ...
    async get() {
        let query = this.client.from(this.colName).select('*');
        
        for (const f of this.filters) {
            const dbField = toSnakeCase(f.field);
            if (f.op === '==' || f.op === '=') {
                query = query.eq(dbField, f.val);
            } else if (f.op === '>') {
                query = query.gt(dbField, f.val);
            }
        }
        
        if (this.orderCol) {
            query = query.order(toSnakeCase(this.orderCol), { ascending: this.orderDirection === 'asc' });
        }
        ...
        const docs = (data || []).map(item => ({
            id: item.id || item.uid,
            exists: true,
            data: () => mapKeysToCamel(item, this.colName)
        }));
...
class SupabaseDocument {
    ...
    async get() {
        ...
        return {
            ...
            data: () => data ? mapKeysToCamel(data, this.colName) : null,
            id: this.docId
        };
    }
    ...
    onSnapshot(onNext, onError) {
        this.get().then(doc => onNext(doc)).catch(err => {
            if (onError) onError(err);
            else console.error(err);
        });

        const channel = this.client
            .channel(`realtime-${this.colName}-${this.docId}`)
            .on(
                'postgres_changes',
                {
                    event: '*',
                ...
                (payload) => {
                    ...
                    onNext({
                        exists: docData !== null && docData !== undefined && Object.keys(docData).length > 0,
                        data: () => docData ? mapKeysToCamel(docData, this.colName) : null,
                        id: this.docId
                    });
                }
            )
...
// Initialize Supabase with Mock Fallback for local sandbox testing
function initFirebase() {
    const isPlaceholderKey = supabaseConfig.url.includes("placeholder") || supabaseConfig.url === "" || supabaseConfig.anonKey.includes("placeholder") || supabaseConfig.anonKey === "";
    
    if (isPlaceholderKey) {
        console.log("Placeholder credentials detected. Starting in mockup mode.");
        setupMockFallbacks();
        return;
    }
    ...
    try {
        if (typeof supabase !== 'undefined' && supabase.createClient) {
            supabaseClientInstance = supabase.createClient(supabaseConfig.url, supabaseConfig.anonKey);
            db = new SupabaseFirestoreAdapter(supabaseClientInstance);
            auth = new SupabaseAuthAdapter(supabaseClientInstance);
            ...
            console.log("Supabase library not found, starting in mockup mode.");
            setupMockFallbacks();
        }
    } catch (err) {
        console.error("Supabase fail, fall back to mock:", err);
        setupMockFallbacks();
    }
}
...
async function handleGoogleLogin(role) {
    if (!auth || !auth.signInWithPopup) {
        // Mock auth Google login fallback for offline testing
        const email = `${role}_google@gmail.com`;
        const u = { email, uid: 'mock_google_uid_' + role, displayName: 'Google User (' + role.toUpperCase() + ')' };
        if (auth) {
            auth.currentUser = u;
        }
        window.mockUserRole = role;
        ...
        try {
            ...
            // Seed mock parent profile if they log in as a parent
            if (role === 'parent') {
                const parentDoc = await db.collection('parents').doc(u.uid).get();
                if (!parentDoc.exists) {
                    await db.collection('parents').doc(u.uid).set({
                        email: email,
                        parentName: u.displayName,
                        phone: "6360660450",
                        studentName: "Aarav Sharma",
                        studentGrade: "Grade 8",
                        studentProgress: 85,
                        attendanceRate: 92,
                        nextSession: "July 12, 2026 - 5:00 PM"
                    });
                }
            }
            handleAuthStateChanged(u);
            ...
            console.error("Mock Google auth Firestore seeding failed:", err);
            handleAuthStateChanged(u);
            navigateToDashboard(role);
        }
        return;
...
async function handleSignupSubmit(event, role) {
    ...
    try {
        ...
        const userProfile = { uid: userCred.user.uid, email, role, name };
...
function renderPortalData(portalId, uid) {
    ...
    if (portalId === 'student') {
        db.collection('users').doc(uid).get().then(doc => {
            if (doc.exists) document.getElementById('student-welcome-name').innerText = `Welcome back, ${doc.data().name || 'Aarav Sharma'}!`;
        });
        ...
        db.collection('users').doc(uid).get().then(doc => {
            if (doc.exists && doc.data().studentEmailLink) {
                const link = doc.data().studentEmailLink;
                db.collection('users').where('email', '==', link).get().then(snap => {
                    document.getElementById('parent-child-name').innerText = !snap.empty ? snap.docs[0].data().name : link;
                });
            }
        });
...
async function seedFirestoreDatabase() {
    ...
    try {
        ...
        await db.collection('community_links').doc('whatsapp').set({ url: "https://wa.me/916360660450" });
    } catch (e) {
...
function setupMockFallbacks() {
    ...
    mockStorage['community_links/whatsapp'] = { url: "https://wa.me/916360660450" };
...
// Switch Day inside the curriculum carousel showcase on the Free Workshop page
function switchCurriculumDay(dayNum) {
    // Deactivate all tabs
    document.querySelectorAll('.carousel-tab').forEach(tab => tab.classList.remove('active'));
    // Deactivate all slides
    document.querySelectorAll('.carousel-image-slide').forEach(slide => slide.classList.remove('active'));
    
    // Activate target tab and slide
    const tabEl = document.querySelector(`.carousel-tabs .carousel-tab:nth-child(${dayNum})`);
    const slideEl = document.getElementById(`slide-day-${dayNum}`);
